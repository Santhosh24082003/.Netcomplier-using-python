...
def compile_and_run_dotnet_code(code, filename="Program.cs"):
    project_dir = 'TempProject'
    if os.path.exists(project_dir):
        shutil.rmtree(project_dir)  # Remove the directory and its contents

    create_command = ['dotnet', 'new', 'console', '-o', project_dir, '--force']
    subprocess.run(create_command, capture_output=True, text=True)

    cs_file_path = os.path.join(project_dir, 'Program.cs')
    with open(cs_file_path, "w") as file:
        file.write(code)

    build_command = ['dotnet', 'build', project_dir, '--no-incremental']
    ...
    if build_result.returncode == 0:
        ...
        exe_filename = os.path.join(project_dir, 'bin', 'Debug', 'net8.0', 'TempProject.dll')

        if not os.path.exists(exe_filename):
            return "Error: The DLL file was not found. Build might have failed."

        run_result = subprocess.run(['dotnet', exe_filename], capture_output=True, text=True)
...
def complex_code():
    ...
    """
    output = compile_and_run_dotnet_code(dotnet_code)
